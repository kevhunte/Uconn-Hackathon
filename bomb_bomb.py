...
game_desc='prototype for the game bomb-bomb. This game will have a grid with objects that light up. The user must repeat the same pattern to beat the level in the allotted time. As they progress, the speed, block sizes, and sequence increase when the time runs out, the game is over'
...
def make_buttons(num,padding):
    ...
    while(count<num):					#repeat until all squares are made
     ...
     #button.config(width=size, height=size)		#button size
     x+=1
     if(x>=edge): 
	y+=1
	x=0
...
load_button = tk.Button(text="Description", bg="blue", command=not_done)	#will run a load function for the game
